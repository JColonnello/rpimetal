...
#define USPI_DEFAULT_KEYMAP_UK
	...
	//
	void *malloc(long unsigned nSize); // result must be 4-byte aligned
...
typedef unsigned long TKernelTimerHandle;
	...
	unsigned StartKernelTimer(
		...
		void *pParam,
		void *pContext
	); // handed over to the timer handler
...
#define LOG_DEBUG 4
	...
	__attribute__((format(printf, 3, 4))) void LogWrite(
		const char *pSource, // short name of module
		unsigned Severity,   // see above
		const char *pMessage,
		...
	); // uses printf format options
