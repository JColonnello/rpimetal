#include "drivers/sd.h" // This units header
...
{--------------------------------------------------------------------------*/
#define waitMicro timer_microsleep         // Waits a number of microseconds
#define TICKCOUNT timer_monotonic          // Gets current system clock value (1Mhz accuracy)
#define TIMEDIFF(start, end) (end - start) // Given two TICKCOUNT values calculates microseconds between them.
...
struct __attribute__((__packed__, aligned(4))) regCMDTM
{
	union
	{
		struct __attribute__((__packed__, aligned(1)))
		{
			...
			volatile unsigned
				TM_DAT_DIR : 1; // @4		Direction of data transfer (0 = host to card , 1 = card to host )
			...
			volatile unsigned
				CMD_IXCHK_EN : 1; // @20		Check that response has same index as command (0=disabled, 1= enabled)
...
struct __attribute__((__packed__, aligned(4))) regCONTROL2
{
	union
	{
		struct __attribute__((__packed__, aligned(1)))
		{
			...
			volatile const unsigned
				ACEND_ERR : 1; // @3		End bit is not 1 during auto command execution **read only
			volatile const unsigned
				ACBAD_ERR : 1;     // @4		Command index error occurred during auto command execution **read only
			unsigned reserved : 2; // @5-6		Write as zero read as don't care
			volatile const unsigned
				NOTC12_ERR : 1;     // @7		Error occurred during auto command CMD12 execution **read only
...
****************************************************************************/
#define EMMC_ARG2 ((volatile __attribute__((aligned(4))) uint32_t *)(uintptr_t)(PBASE + 0x300000))
#define EMMC_BLKSIZECNT ((volatile struct __attribute__((aligned(4))) regBLKSIZECNT *)(uintptr_t)(PBASE + 0x300004))
#define EMMC_ARG1 ((volatile __attribute__((aligned(4))) uint32_t *)(uintptr_t)(PBASE + 0x300008))
#define EMMC_CMDTM ((volatile struct __attribute__((aligned(4))) regCMDTM *)(uintptr_t)(PBASE + 0x30000c))
#define EMMC_RESP0 ((volatile __attribute__((aligned(4))) uint32_t *)(uintptr_t)(PBASE + 0x300010))
#define EMMC_RESP1 ((volatile __attribute__((aligned(4))) uint32_t *)(uintptr_t)(PBASE + 0x300014))
#define EMMC_RESP2 ((volatile __attribute__((aligned(4))) uint32_t *)(uintptr_t)(PBASE + 0x300018))
#define EMMC_RESP3 ((volatile __attribute__((aligned(4))) uint32_t *)(uintptr_t)(PBASE + 0x30001C))
#define EMMC_DATA ((volatile __attribute__((aligned(4))) uint32_t *)(uintptr_t)(PBASE + 0x300020))
#define EMMC_STATUS ((volatile struct __attribute__((aligned(4))) regSTATUS *)(uintptr_t)(PBASE + 0x300024))
#define EMMC_CONTROL0 ((volatile struct __attribute__((aligned(4))) regCONTROL0 *)(uintptr_t)(PBASE + 0x300028))
#define EMMC_CONTROL1 ((volatile struct __attribute__((aligned(4))) regCONTROL1 *)(uintptr_t)(PBASE + 0x30002C))
#define EMMC_INTERRUPT ((volatile struct __attribute__((aligned(4))) regINTERRUPT *)(uintptr_t)(PBASE + 0x300030))
#define EMMC_IRPT_MASK ((volatile struct __attribute__((aligned(4))) regIRPT_MASK *)(uintptr_t)(PBASE + 0x300034))
#define EMMC_IRPT_EN ((volatile struct __attribute__((aligned(4))) regIRPT_EN *)(uintptr_t)(PBASE + 0x300038))
#define EMMC_CONTROL2 ((volatile struct __attribute__((aligned(4))) regCONTROL2 *)(uintptr_t)(PBASE + 0x30003C))
#define EMMC_TUNE_STEP ((volatile struct __attribute__((aligned(4))) regTUNE_STEP *)(uintptr_t)(PBASE + 0x300088))
#define EMMC_SLOTISR_VER ((volatile struct __attribute__((aligned(4))) regSLOTISR_VER *)(uintptr_t)(PBASE + 0x3000fC))
...
struct __attribute__((__packed__, aligned(4))) regSCR
{
	union
	{
		struct __attribute__((__packed__, aligned(1)))
		{
			...
			volatile unsigned
				DATA_AFTER_ERASE : 1; // @15		Defines the data status after erase, whether it is 0 or 1
...
struct __attribute__((__packed__, aligned(4))) regCID
{
	...
	union
	{
		struct __attribute__((__packed__, aligned(1)))
		{
			...
			volatile unsigned
				ManufactureYear : 8; // @4-11	Manufacturing dateyear (offset from 2000 .. 1=2001,2=2002,3=2003 etc)
...
#define INT_ERROR_MASK \
	(INT_CRC_ERROR | INT_END_ERROR | INT_INDEX_ERROR | INT_DATA_TIMEOUT | INT_DATA_CRC_ERR | INT_DATA_END_ERR | \
...
#define ACMD41_ARG_HC \
	(ACMD41_HCS | ACMD41_SDXC_POWER | ACMD41_VOLTAGE) //(ACMD41_HCS|ACMD41_SDXC_POWER|ACMD41_VOLTAGE|ACMD41_S18R)
...
static EMMCCommand sdCommandTable[IX_SEND_SCR + 1] = {
	...
	[IX_READ_SINGLE] =
		{"READ_SINGLE",
		 .code.CMD_INDEX = 0x11,
		 .code.CMD_RSPNS_TYPE = CMD_48BIT_RESP,
		 .code.CMD_ISDATA = 1,
		 .code.TM_DAT_DIR = 1,
		 .use_rca = 0,
		 .delay = 0},
...
{
	LOG_DEBUG(
		"EMMC: Status: %08x, control1: %08x, interrupt: %08x\n",
		(unsigned int)EMMC_STATUS->Raw32,
		(unsigned int)EMMC_CONTROL1->Raw32,
		(unsigned int)EMMC_INTERRUPT->Raw32
	);
	LOG_DEBUG(
		"EMMC: Command %s resp %08x: %08x %08x %08x %08x\n",
		sdCard.lastCmd->cmd_name,
		(unsigned int)resp,
		(unsigned int)*EMMC_RESP3,
		(unsigned int)*EMMC_RESP2,
		(unsigned int)*EMMC_RESP1,
...
static SDRESULT sdWaitForInterrupt(uint32_t mask)
{
	...
	{
		if (!start_time)
			start_time = TICKCOUNT(); // If start time not set the set start time
		else
			td = TIMEDIFF(start_time, TICKCOUNT()); // Time difference between start time and now
	}
		...
		(ival & INT_DATA_TIMEOUT))         // Data timeout occurred
	{
		if (LOG_ERROR)
			LOG_ERROR(
				"EMMC: Wait for interrupt %08x timeout: %08x %08x %08x\n",
				(unsigned int)mask,
				(unsigned int)EMMC_STATUS->Raw32,
				(unsigned int)ival,
				(unsigned int)*EMMC_RESP0
			); // Log any error if requested
	...
	else if (ival & INT_ERROR_MASK)
	{
		if (LOG_ERROR)
			LOG_ERROR(
				"EMMC: Error waiting for interrupt: %08x %08x %08x\n",
				(unsigned int)EMMC_STATUS->Raw32,
				(unsigned int)ival,
				(unsigned int)*EMMC_RESP0
			); // Log any error if requested
...
static SDRESULT sdWaitForCommand(void)
{
	...
	{
		if (!start_time)
			start_time = TICKCOUNT(); // Get start time
		else
			td = TIMEDIFF(start_time, TICKCOUNT()); // Time difference between start and now
	}
	if ((td >= 1000000) || (EMMC_INTERRUPT->Raw32 & INT_ERROR_MASK)) // Error occurred or it timed out
	{
		if (LOG_ERROR)
			LOG_ERROR(
				"EMMC: Wait for command aborted: %08x %08x %08x\n",
				(unsigned int)EMMC_STATUS->Raw32,
				(unsigned int)EMMC_INTERRUPT->Raw32,
				(unsigned int)*EMMC_RESP0
			);          // Log any error if requested
...
static SDRESULT sdWaitForData(void)
{
	...
	if ((td >= 500000) || (EMMC_INTERRUPT->Raw32 & INT_ERROR_MASK))
	{
		if (LOG_ERROR)
			LOG_ERROR(
				"EMMC: Wait for data aborted: %08x %08x %08x\n",
				(unsigned int)EMMC_STATUS->Raw32,
				(unsigned int)EMMC_INTERRUPT->Raw32,
				(unsigned int)*EMMC_RESP0
			);          // Log any error if requested
...
static void unpack_csd(struct CSD *csd)
{
	...
	*p = *EMMC_RESP3;
	...
	LOG_DEBUG(
		"CSD Contents : %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
		buf[2],
		buf[1],
		buf[0],
		buf[7],
	...
	LOG_DEBUG(
		"  csd_structure=%d\t  spec_vers=%d\t  taac=%02x\t nsac=%02x\t  tran_speed=%02x\t  ccc=%04x\n"
	...
	{
		LOG_DEBUG(
			"CSD 1.0: c_size = %d\t  c_size_mult=%d\t card capacity: %lu\n"
...
static int sdSendCommandP(EMMCCommand *cmd, uint32_t arg)
{
	...
	*EMMC_CMDTM = cmd->code; // Send command to SD card
	if (cmd->delay)
		waitMicro(cmd->delay); // Wait for required delay
...
static SDRESULT sdReadSCR(void)
{
	...
	while (numRead < 2)
	{
		if (EMMC_STATUS->READ_TRANSFER)
		{
			...
			if (numRead == 0)
				sdCard.scr.Raw32_Lo = *EMMC_DATA;
			else
				sdCard.scr.Raw32_Hi = *EMMC_DATA;
			numRead++;
		}
		else
		{
...
SDRESULT sdTransferBlocks(uint32_t startBlock, uint32_t numBlocks, uint8_t *buffer, bool write)
{
	if (sdCard.type == SD_TYPE_UNKNOWN)
		return SD_NO_RESP; // If card not known return error
	if (sdWaitForData())
	...
	int transferCmd =
		write ? (numBlocks == 1 ? IX_WRITE_SINGLE : IX_WRITE_MULTI) : (numBlocks == 1 ? IX_READ_SINGLE : IX_READ_MULTI);
	...
	if ((numBlocks > 1) && (sdCard.scr.CMD_SUPPORT != CMD_SUPP_SET_BLKCNT) && (resp = sdSendCommand(IX_STOP_TRANS)))
		return sdDebugResponse(resp);
...
SDRESULT sdClearBlocks(uint32_t startBlock, uint32_t numBlocks)
{
	...
	uint32_t endAddress =
		sdCard.type == SD_TYPE_2_SC ? (uint32_t)((startBlock + numBlocks) << 9) : (uint32_t)(startBlock + numBlocks);
	SDRESULT resp;
	...
	while (EMMC_STATUS->DAT_INHIBIT)
	{
		if (--count == 0)
		{
			if (LOG_ERROR)
				LOG_ERROR(
					"EMMC: Timeout waiting for erase: %08x %08x\n",
					(unsigned int)EMMC_STATUS->Raw32,
					(unsigned int)EMMC_INTERRUPT->Raw32
				);
			return SD_TIMEOUT;
...
struct __attribute__((packed, aligned(1))) BPBFAT32_struct
{
	...
	uint16_t
		ExtFlags; // 40-41   (Mirror flags Bits 0-3: number of active FAT (if bit 7 is 1) Bit 7: 1 = single active FAT; zero: all FATs are updated at runtime; Bits 4-6 & 8-15 : reserved)
...
struct __attribute__((packed, aligned(4))) FAT_BPB_struct
{                                 // OFFSET LOCATIONS
	...
	union
	{                                     // Unionize the two structures at offset 36
...
struct __attribute__((packed, aligned(2))) partition_info
{
	...
	uint8_t
		type; // partition type (01h = 12bit FAT, 04h = 16bit FAT, 05h = Ex MSDOS, 06h = 16bit FAT (>32Mb), 0Bh = 32bit FAT (<2048GB))
...
static bool LoadDrivePartition(printhandler prn_basic)
{
	...
	{                                                         // Check if it is boot sector
		...
		if (mbr->signature != 0xaa55)
			return false;                                        // if it is not even MBR then it's not FAT
	...
	else
	{
		...
		sdCard.partition.dataSectors =
			bpb->TotalSectors32 - (bpb->NumFATs * bpb->FATSize16) - 33; // -1 see above +1 and 32 fixed sectors
	...
	if (prn_basic)
		prn_basic(
			"First Sector: %lu, Data Sectors: %lu, TotalClusters: %lu, RootCluster: %lu\n",
			(unsigned long)sdCard.partition.firstDataSector,
			(unsigned long)sdCard.partition.dataSectors,
			(unsigned long)partition_totalClusters,
			(unsigned long)sdCard.partition.rootCluster
		);
	return true;
}
...
uint32_t getSetNextCluster(uint32_t clusterNumber, bool set, uint32_t clusterEntry)
{
	...
	uint8_t buffer[512];
	FATEntrySector =
		sdCard.partition.unusedSectors + sdCard.partition.reservedSectorCount +
		((clusterNumber * 4) / sdCard.partition.bytesPerSector); // Get sector number of the cluster entry in the FAT
	FATEntryOffset =
		((clusterNumber * 4) % sdCard.partition.bytesPerSector); // Get the offset address in that sector number
	if (sdTransferBlocks(FATEntrySector, 1, (uint8_t *)&buffer[0], false) != SD_OK)
	{
		return 0xFFFFFFFF; // Sector read failed twice so fail exit
	}
	FATEntryValue =
		(uint32_t *)&buffer[FATEntryOffset]; // Get the cluster address from the buffer .. fortunately always aligned
	if (set == false)
		return (*FATEntryValue);   // If not setting exit with the retrieved value
...
static void SetFindDataFromFATEntry(struct dir_Structure *dir, FIND_DATA *lpFD)
{
	if ((dir) && (lpFD))
	{
		...
		lpFD->CreateDT.tm_year = ((dir->createDate & 0xFE00) >> 9) +
								 80; // Looks weird but file date is from 1980 and C standard from 1900 so add 80
		...
		lpFD->LastAccessDate.tm_year = ((dir->lastAccessDate & 0xFE00) >> 9) +
									   80; // Looks weird but file date is from 1980 and C standard from 1900
		lpFD->LastAccessDate.tm_mon =
			((dir->lastAccessDate & 0x01E0) >> 5) - 1;                 // file date is 1-12 .. C standard is 0-11
		lpFD->LastAccessDate.tm_mday = (dir->lastAccessDate & 0x001F); // day is strangely correct both 1-31

		lpFD->WriteDT.tm_year =
			((dir->writeDate & 0xFE00) >> 9) + 80; // Looks weird but file date is from 1980 and C standard from 1900
	...
	{ // Check the directory pointer valid
		...
		{ // For each of the first 5 characters
			...
			if (utf == 0)
				...
				LFNtext[LFN_blockcount++] = (char)wctob(utf); // Otherwise narrow to ascii and place in buffer
		}
...
bool WildcardMatch(const char *pattern, const char *text)
{
	...
	int ch;
	bool found;
	while (toupper(*text) || toupper(*pattern))
	{                    // While neither string terminated
		...
		case '[':                            // Multi character select wildcard open
			...
			{
				if (ch == '\\')
					ch = *pattern++; // Literal request
				if (ch == '\0')
					return false; // Pattern terminated abnormally without match
				if (*text == ch)
					found = false; // character not found
			}
		...
		case '?': // Single character wildcard
			if (*text++ == '\0')
				return false; // text string ended so no match
			break;
		case '\\':           // Literal slash
		...
		default:
			...
			{
				if (*text)
					text++; // Move the text pointer forward
				break;
			}
...
static struct dir_Structure *LocateFATEntry(
	const char *searchPat, struct PRIV_SEARCH_DATA *priv, bool dirChange, LFN_NAME LFN_Name, uint32_t *ErrorID
)
{
	struct dir_Structure *dir;
	uint32_t LFN_count = 0;
	while ((priv->cluster < 0x0ffffff6) && (priv->cluster != 0))
	{ // Check cluster valid and read successful
		while (priv->sector < sdCard.partition.sectorPerCluster)
		{
			while (priv->bPos < 512)
			{                                                            // While not a buffer end
				dir = (struct dir_Structure *)&priv->buffer[priv->bPos]; // Transfer buffer position to pointer
				if (dir->name[0] == FILE_EMPTY)
				{
					if (ErrorID)
						*ErrorID = FAT_END_REACHED; // End of FAT entry chain reached
					return NULL;                    // Return null pointer
				}
				if (dir->name[0] != FILE_DELETED)
				{ // If entry not deleted
					if (dir->attrib == FILE_ATTRIBUTE_LABEL)
					{   // FAT LABEL ENTRY
					...
					else
					{
						...
						{
							if (dotadded)
								LFN_Name[dotadded] = '\0';              // Make asciiz
							priv->bPos += sizeof(struct dir_Structure); // Move pointer down
							if (ErrorID)
								*ErrorID = FAT_RESULT_OK; // Return result of FAT_RESULT_OK if requested
			...
			if (priv->sector < sdCard.partition.sectorPerCluster)
			{                   // Next sector valid do disk read
				priv->bPos = 0; // Reset buffer position to top of buffer
				if (sdTransferBlocks(priv->firstSector + priv->sector, 1, &priv->buffer[0], false) != SD_OK)
				{
					if (ErrorID)
						*ErrorID = FAT_READSECTOR_FAIL; // Check for read failure
					return NULL;                        // Return null pointer
				}
			}
		}
		priv->cluster = getSetNextCluster(priv->cluster, false, 0); // Fetch the next cluster
		priv->firstSector = getFirstSector(
			priv->cluster,
			sdCard.partition.sectorPerCluster,
			sdCard.partition.firstDataSector
		);                // Hold the first sector of this new cluster
	...
	if (ErrorID)
		*ErrorID = FAT_INVALID_DATAPTR; // If valid pointer return error id
...
static bool CopyUnAlignedString(char *Dest, const char *Src)
{
	...
	{
		while (*Src)
			*Dest++ = *Src++; // Transfer source byte to dest and increment pointers
...
HANDLE sdFindFirstFile(const char *lpFileName, FIND_DATA *lpFFData)
{
	...
	if ((lpFileName == 0) || (lpFFData == 0))
		return 0;                                  // One of the pointers is invalid so fail
	...
	if (searchStr[0] == '\\')
		searchStr++; // We sometimes write root directory with leadslash .. remove it
	while ((intSearchRecord[i].firstSector != 0) && i < MAX_SEARCH_RECORDS)
		i++;
	if (i != MAX_SEARCH_RECORDS)
	{ // Empty search record found
		intSearchRecord[i].cluster =
			sdCard.partition.rootCluster; // We are going to start on FAT which starts at rootCluster
		intSearchRecord[i].firstSector = getFirstSector(
			sdCard.partition.rootCluster,
			sdCard.partition.sectorPerCluster,
			sdCard.partition.firstDataSector
		);                             // Hold the first sector of FAT32
		intSearchRecord[i].sector = 0; // Zero sector count
		intSearchRecord[i].bPos = 0;   // Zero buffer position
		if (sdTransferBlocks(intSearchRecord[i].firstSector, 1, &intSearchRecord[i].buffer[0], false) == SD_OK)
		{
			char *p;
			do
			{
				...
				if (p)
				{              // Directory slash found
					*p = '\0'; // Turn the slash into a terminate
					dir = LocateFATEntry(
						searchStr, &intSearchRecord[i], true, lpFFData->cFileName, &errID
					); // Locate directory FAT entry
					if (dir == NULL)
						return 0; // Did not find subdirectory
					intSearchRecord[i].cluster = (((uint32_t)dir->firstClusterHI) << 16) | dir->firstClusterLO;
					intSearchRecord[i].firstSector = getFirstSector(
						intSearchRecord[i].cluster,
						sdCard.partition.sectorPerCluster,
						sdCard.partition.firstDataSector
					);                             // Hold the first sector
					intSearchRecord[i].sector = 0; // Zero sector count
					intSearchRecord[i].bPos = 0;   // Buffer pos starts at top of buffer
					if (sdTransferBlocks(intSearchRecord[i].firstSector, 1, &intSearchRecord[i].buffer[0], false) !=
						SD_OK)
			...
			dir = LocateFATEntry(
				searchStr, &intSearchRecord[i], false, lpFFData->cFileName, &errID
			); // Locate first FAT entry
			if ((errID == FAT_RESULT_OK) && (dir))
			{ // First FAT entry returned
				int j = 0;
				while (searchStr[j] != 0)
				{
...
HANDLE sdFindNextFile(HANDLE hFindFile, FIND_DATA *lpFFData)
{
	uint32_t errID = FAT_RESULT_OK;
	struct dir_Structure *dir;
	if ((hFindFile > 0) && (hFindFile <= MAX_SEARCH_RECORDS) && (intSearchRecord[hFindFile - 1].firstSector != 0))
	{ // File handle maps to an internal search record in use
		dir = LocateFATEntry(
...
.--------------------------------------------------------------------------*/
HANDLE sdCreateFile(
	const char *lpFileName,         // Filename or device to open
...
) // Currently not supported (use 0)
{
	...
	LFN_NAME tempName; // We will cut and chop name so we need local copy
	if (lpFileName == 0)
		return 0;                                  // Name pointer is invalid so fail
	CopyUnAlignedString(&tempName[0], lpFileName); // Make local copy of name as we will chop and change it
	const char *searchStr = &tempName[0];          // Search string starts as lpFilename
	if (searchStr[0] == '\\')
		searchStr++; // We sometimes write root directory with lead slash .. remove it
	while ((intFileIORecord[i].srec.firstSector != 0) && i < MAX_FILE_IO_RECORDS)
		i++;
	if (i != MAX_FILE_IO_RECORDS)
	{ // Empty search record found
		...
		if (sdTransferBlocks(intFileIORecord[i].srec.firstSector, 1, &intFileIORecord[i].srec.buffer[0], false) ==
			SD_OK) // Read a sector of data
		{
			char *p;
			do
			{
				...
				{              // Directory slash found
					*p = '\0'; // Turn the slash into a terminate
					dir = LocateFATEntry(
						searchStr, &intFileIORecord[i].srec, true, openName, &errID
					); // Locate directory FAT entry
					if (dir == NULL)
						return 0; // Did not find subdirectory
					...
					if (sdTransferBlocks(
							intFileIORecord[i].srec.firstSector, 1, &intFileIORecord[i].srec.buffer[0], false
						) != SD_OK)
						return 0;      // Some read error occured
					searchStr = p + 1; // Search string now starts after directory
				}
			...
			dir = LocateFATEntry(
				searchStr, &intFileIORecord[i].srec, false, openName, &errID
			); // Locate file first FAT entry
			if ((errID == FAT_RESULT_OK) && (dir))
			{ // First FAT matching entry returned
...
.--------------------------------------------------------------------------*/
bool sdReadFile(
	HANDLE hFile,                  // Handle as returned from CreateFile
...
{
	if ((hFile > 0) && (hFile <= MAX_FILE_IO_RECORDS) && (intFileIORecord[hFile - 1].srec.firstSector != 0))
	{                                      // File handle maps to an internal file io record in use
		...
		while ((retErrId == FAT_RESULT_OK) && (intFileIORecord[hFile - 1].srec.cluster < 0x0ffffff6) &&
			   (intFileIORecord[hFile - 1].srec.cluster != 0))
		{ // Check cluster valid and read successful

			while ((nNumberOfBytesToRead > bytesRead) && // Still more bytes to read
				   (intFileIORecord[hFile - 1].filePos < intFileIORecord[hFile - 1].fileSize
				   ) &&                                          // Still inside fileSize
				   (intFileIORecord[hFile - 1].srec.bPos < 512)) // While not a buffer end
			{
				((uint8_t *)lpBuffer)[bytesRead] =
					intFileIORecord[hFile - 1]
						.srec.buffer[intFileIORecord[hFile - 1].srec.bPos++]; // Transfer byte and increment pointer
			...
			if (intFileIORecord[hFile - 1].srec.sector <
				sdCard.partition.sectorPerCluster) // Next sector still in current cluster
			{
				intFileIORecord[hFile - 1].srec.bPos = 0; // Reset buffer position to top of buffer
				if (sdTransferBlocks(
						intFileIORecord[hFile - 1].srec.firstSector + intFileIORecord[hFile - 1].srec.sector,
						1,
						&intFileIORecord[hFile - 1].srec.buffer[0],
						false
					) != SD_OK)
				{
			...
			{ // Need to move to next cluster
				intFileIORecord[hFile - 1].srec.cluster =
					getSetNextCluster(intFileIORecord[hFile - 1].srec.cluster, false, 0); // Fetch the next cluster
...
) // FILE_BEGIN, FILE_CURRENT, FILE_END
{
	...
	{ // File handle maps to an internal file io record in use
		...
		if (sdTransferBlocks(
				intFileIORecord[hFile - 1].srec.firstSector, 1, &intFileIORecord[hFile - 1].srec.buffer[0], false
			) != SD_OK)
		{
			return INVALID_SET_FILE_POINTER; // Return set file position failure
		}

		while (lDistanceToMove > 512)
		{                                             // Position not in current sector
			intFileIORecord[hFile - 1].srec.sector++; // Increment sector
			if (intFileIORecord[hFile - 1].srec.sector <
				sdCard.partition.sectorPerCluster) // Next sector still in current cluster
			{
		...
		if (lpDistanceToMoveHigh)
			*lpDistanceToMoveHigh = 0;               // We don't use this
...
SDRESULT sdInitCard(printhandler prn_basic, printhandler prn_error, bool mount)
{
	...
	if (prn_basic)
		prn_basic(
			"EMMC: SD Card %s %dMb mfr %d '%c%c:%c%c%c%c%c' r%d.%d %d/%d, #%08x RCA %04x\n",
			SD_TYPE_NAME[sdCard.type],
			(int)(sdCard.CardCapacity >> 20),
			sdCard.cid.MID,
			sdCard.cid.OID_Hi,
			sdCard.cid.OID_Lo,
			sdCard.cid.ProdName1,
			sdCard.cid.ProdName2,
			sdCard.cid.ProdName3,
			sdCard.cid.ProdName4,
			sdCard.cid.ProdName5,
			sdCard.cid.ProdRevHi,
			sdCard.cid.ProdRevLo,
			sdCard.cid.ManufactureMonth,
			2000 + sdCard.cid.ManufactureYear,
			serial,
			sdCard.rca >> 16
		);

	if (mount)
		if (!LoadDrivePartition(prn_basic))
...
{
	if (sdCard.type != SD_TYPE_UNKNOWN)
		return (&sdCard.csd); // Card success so return structure pointer
	else
		return NULL; // Return fail result of null
}